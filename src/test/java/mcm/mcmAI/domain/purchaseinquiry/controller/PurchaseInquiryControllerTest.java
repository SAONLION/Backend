...
@Transactional
class PurchaseInquiryControllerTest extends AbstractIntegrationTest {

    private static final AtomicLong SKU_ID_SEQUENCE = new AtomicLong(970_000_000L);
    ...
    @Test
    void 구매_문의가_정상적으로_생성된다() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/sessions/{sessionId}/purchase-inquiries", session.getSessionId())
                ...
                .andExpect(jsonPath("$.purchaseInquiryId").exists())
                .andExpect(jsonPath("$.sku").value(sku.getSku()))
                .andExpect(jsonPath("$.inquiredAt").exists());
    }
    ...
    void 존재하지_않는_sessionId면_404와_SESSION_NOT_FOUND_코드를_반환한다() throws Exception {
        mockMvc.perform(post("/api/v1/sessions/{sessionId}/purchase-inquiries", "not-exist-session")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"sku\": 1}"))
                .andExpect(status().isNotFound())
