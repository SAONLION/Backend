...
class TryonRequestControllerTest extends AbstractIntegrationTest {
    ...
    @Test
    void 착장_요청과_연결된_직원_호출이_SA_보드에_노출되고_완료처리된다() throws Exception {
        Session session = newSession();
        Sku sku = newSku(newProduct());
        ...
        String boardResponse = mockMvc.perform(get("/api/v1/staff/staff-calls")
                ...
                .andExpect(jsonPath("$.waiting[0].reason").value("착용 요청"))
                ...
                .andExpect(jsonPath("$.waiting[0].size").value(sku.getSize()))
